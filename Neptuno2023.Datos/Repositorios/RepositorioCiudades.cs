...
//c.CiudadId, c.NombreCiudad, c.PaisId, p.NombrePais  
//                                        FROM Ciudades c 
//                                        ineer join Paises p ON c.PaisId=p.PaisId
//                                        ORDER BY PaisId, NombreCiudad 
        ...
        public bool EstaRelacionada(Ciudad ciudad)
        {
            ...
            {
                conn.Open();
                string selectQuery = @"SELECT(SELECT COUNT(*) FROM Clientes WHERE PaisId=@PaisId)
                                +(SELECT COUNT(*) FROM Proveedores WHERE PaisId=@PaisId)";
                using (var comando = new SqlCommand(selectQuery, conn))
                {

                    comando.Parameters.Add("@PaisId", SqlDbType.Int);
                    comando.Parameters["@PaisId"].Value = ciudad.CiudadId;
