...
namespace Neptuno2023.Datos.Repositorios
{
    public class RepositorioProveedores : IRepositorioProveedores
    {
        ...
        public List<ProveedorDto> GetProveedorDto()
        {
            ...
            {
                con.Open();
                string selectQuery = @"SELECT p.ProveedorId, p.RazonSocial, p.PaisId, pa.NombrePais, p.CiudadId ,ci.NombreCiudad
                        FROM Proveedores p
                        INNER JOIN Paises pa ON p.PaisId=pa.PaisId
                        INNER JOIN Ciudades ci ON p.CiudadId=ci.CiudadId
        ...
        private ProveedorDto ConstruirProveedorDto(SqlDataReader reader)
        {
            return new ProveedorDto()
            {
                ...
                NombreCiudad = reader.GetString(5)
